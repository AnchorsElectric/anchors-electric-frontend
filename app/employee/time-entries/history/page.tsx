...
export default function EmployeeTimeEntriesHistoryPage() {
  ...
  const loadTimeEntries = async () => {
    try {
      setLoading(true);
      setError('');
      console.log('Loading time entries history with filter:', statusFilter);
      const response = await apiClient.getMyTimeEntriesHistory({ 
      ...
      if (response.success && response.data) {
        ...
        entries.forEach((entry: any) => {
          console.log(`Week ${entry.weekRange}: ${entry.entries?.length || 0} day entries`);
          if (entry.entries) {
            entry.entries.forEach((dayEntry: any) => {
              console.log(`  - ${dayEntry.date}: ${dayEntry.status} (ID: ${dayEntry.id})`);
            });
          }
        });
      ...
      console.error('Error loading time entries history:', err);
                      ...
                      {isExpanded && (
                        <>
                          <tr className={styles.dayHeaderRow}>
                            <td colSpan={5} className={styles.dayHeader}>
                                ...
                                <React.Fragment key={dayEntry.id}>
                                  <tr className={styles.dayRow}>
                                    <td>
                                      <div className={styles.expandableCell}>
                                        ...
                                        <div>{formatDate(dayEntry.date)}</div>
                                      </div>
                                    </td>
                                    <td>
                                      {entry.paymentType === 'HOURLY' 
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        ...
                                        {isRejected && dayEntry.rejectionReason && (
                                          <button
                                            ...
                                            className={styles.rejectionToggle}
                                            style={{
                                              background: 'none',
                                              border: 'none',
                                              cursor: 'pointer',
                                  ...
                                  {isRejected && isRejectedExpanded && dayEntry.rejectionReason && (
                                    <tr className={styles.rejectionReasonRow}>
                                      <td colSpan={5} className={styles.rejectionReasonCell}>
