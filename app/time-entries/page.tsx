...
export default function TimeEntriesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const filteredEntries = entries.filter((e: any) => e.status !== 'REJECTED');
        console.log('Time entries received from backend:', filteredEntries);
        console.log('Number of entries:', filteredEntries.length);
        filteredEntries.forEach((entry: any, index: number) => {
          console.log(`Entry ${index + 1}:`, {
            id: entry.id,
            date: entry.date,
            hours: entry.hours,
            hoursType: typeof entry.hours,
            status: entry.status,
          });
        });
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to save time entry');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className={styles.calendar}>
            ...
            <div className={styles.days}>
              {days.map((date, index) => {
                ...
                if (entry) {
                  console.log('Rendering entry for date:', dateStr, 'Entry:', entry);
                }
                const today = new Date();
                ...
                // Today should be clickable, so isPast should be false for today
                ...
                console.log('Date check:', {
                  ...
                  todayStr,
                  isToday,
                  dateMidnight: dateMidnight.toISOString(),
                  ...
                  >
                    ...
                    {entry && (
                      <div className={styles.dayEntry}>
                        {(() => {
                          console.log('Rendering entry for date:', dateStr, {
                            paymentType: employeeProfile.paymentType,
                            hours: entry.hours,
                            hoursType: typeof entry.hours,
                            hoursIsNull: entry.hours === null,
                            hoursIsUndefined: entry.hours === undefined,
                            shouldShowHours: employeeProfile.paymentType === 'HOURLY' && entry.hours !== null && entry.hours !== undefined,
                          });
                          return null;
                        })()}
